local t = Def.ActorFrame{
    OnCommand=function(self)
        -- Module to use Lua input on a screen without having to make a entire function.
        -- For more information, check _fallback/Modules/Lua.InputSystem.lua.
...
-- Simple background to make the text more visible
t[#t+1] = Def.Quad{
    OnCommand=function(self)
        self:stretchto( SCREEN_WIDTH, SCREEN_HEIGHT, 0, 0  ):diffuse( Color.Black ):diffusealpha(0.8)
    end
}

-- These lines are in specification in order to fit with the Common Normal font.
-- You'll need to adjust in order to make them fit with your theme if you're planning on keeping the same
-- help menu layout.
local textzoom = 0.6 * (SCREEN_HEIGHT/480)

for k,v in ipairs( HelpLines ) do
    local HelpLine = Def.ActorFrame{
        OnCommand=function(self)
            self:y( SAFE_HEIGHT-6 + ( (14 * (SCREEN_HEIGHT/480)) * (k-1) )  )
        end
    }
    ...
    -- Some buttons will have a secondary option, so don't create the actor if there isn't any.
    if v[3] then
