	...
	if filterAlphas[player] > 0 then
		t[#t+1] = Def.Quad {
			...
			OnCommand=function(self)
				if GAMESTATE:GetIsFieldCentered(pNum-1) then
					pos = SCREEN_CENTER_X
				end
				...
				self:x(pos):CenterY():zoomto(GAMESTATE:GetStyleFieldSize(pNum-1) * 1.5 + padding,SCREEN_HEIGHT)
	...
	if styleType == "TwoPlayersSharedSides" then
		...
		if filterAlphas[player] > 0 then
			t[#t+1] = Def.Quad{
				Name="RoutineFilter",
				OnCommand=function(self) 
					self:x(THEME:GetMetric("ScreenGameplay",metricName)):CenterY():zoomto(GAMESTATE:GetStyleFieldSize(pNum-1) * 1.5 + padding,SCREEN_HEIGHT):diffusecolor(filterColorsA[sfcChoice]):diffusealpha(filterAlphas[player])
				end
			}
		end
	else
