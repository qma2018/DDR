...
function USWN.Load()
	...
	if string.find(sElement, "HitMine Explosion") then
		Button = "Fallback";
	end

	-- Set up receptors
	if string.find(sButton,"RightFoot") and (string.find(sElement, "Receptor") 
	or string.find(sElement, "Explosion")
	or string.find(sElement, "Lift")) then
		Button = "AnyRightFoot"
	end
	if string.find(sButton,"LeftFoot") and (string.find(sElement, "Receptor") 
	or string.find(sElement, "Explosion")
	or string.find(sElement, "Lift")) then
		Button = "AnyLeftFoot"
	end
	if string.find(sButton,"RightFist") and (string.find(sElement, "Receptor") 
	or string.find(sElement, "Explosion")
	or string.find(sElement, "Lift")) then
		Button = "AnyRightFist"
	end
	if string.find(sButton,"LeftFist") and (string.find(sElement, "Receptor") 
	or string.find(sElement, "Explosion")
	or string.find(sElement, "Lift")) then
		Button = "AnyLeftFist"
	end

	--- Set up receptors for other styles
