...
function Nskin.Load()
	...
	local diff = color("1,0,0,1")
	
	if string.find(sElement, "P2") then diff = color("0,0,1,1") sElement = sElement:gsub("P2 ", "") end
	if string.find(sElement, "P3") then diff = color("1,1,0,1") sElement = sElement:gsub("P3 ", "") end
	if string.find(sElement, "P4") then diff = color("0,1,0,1") sElement = sElement:gsub("P4 ", "") end
	if string.find(sElement, "P5") then diff = color(".5,0,.5,1") sElement = sElement:gsub("P5 ", "") end
	if string.find(sElement, "P6") then diff = color("1,.5,0,1") sElement = sElement:gsub("P6 ", "") end
	if string.find(sElement, "P7") then diff = color("1,1,1,1") sElement = sElement:gsub("P7 ", "") end
	if string.find(sElement, "P8") then diff = color(".2,.2,.2,1") sElement = sElement:gsub("P8 ", "") end
	if string.find(sElement, "P9") then diff = color("1,0,1,1") sElement = sElement:gsub("P9 ", "") end
	if string.find(sElement, "P10") then diff = color(".5,0,0,1") sElement = sElement:gsub("P10 ", "") end
			
	if string.find(sElement, "Explosion") or string.find(sElement, "Receptor") then diff = color("1,1,1,1") end
	...
	local t = Def.ActorFrame{
		Def.ActorFrame{
			InitCommand=function(self) self:diffuse(diff) end,
